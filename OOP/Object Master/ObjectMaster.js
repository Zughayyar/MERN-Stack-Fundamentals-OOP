...
const fireTypePokemon = pokémon.filter(p => p.types.includes("fire"));
console.log(fireTypePokemon);

// 3. An array of Pokémon objects that have more than one type
const multipleTypesPokemon = pokémon.filter(p => p.types.length > 1);
...
const namesWithHighId = pokémon.filter(p => p.id > 99).map(p => p.name);
console.log(namesWithHighId);

// 6. An array with just the names of the Pokémon whose only type is poison
const poisonOnlyPokemon = pokémon
...
const firstTypeFlyingSecond = pokémon
